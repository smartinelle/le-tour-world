...
| --- | --- | --- | --- | --- |
| Product positioning | App is presented as le-tour, an early-user local-first solo training app. | Public-facing name, docs, env vars, app title, config paths, export names, and the Python namespace have been renamed. The Python import package is `le_tour` because hyphenated module names are invalid. | Ready | Keep new user-facing surfaces consistently branded as le-tour. |
...
| Settings device pairing | Rider can scan, select, connect, disconnect, and understand trainer state. | Device pairing lives under Settings. Scan dialogs exist for trainer and HR. `TrainerService` wraps FTMS client. Auto-connect is gated by `LE_TOUR_ENABLE_BLE`. | Partial | Hardware-test with real trainers, improve retry/reconnect, document OS Bluetooth permissions, clarify demo-vs-live state, and resolve/verify FTMS command TODOs. |
...
- How much of the unavoidable Python package name `le_tour` should be exposed
  in early-user docs versus kept to developer-facing references only?
