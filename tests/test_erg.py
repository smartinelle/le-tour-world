...
class TestErgController:
    ...
    def test_step_response(self):
        ...
        for i in range(10):
            output = controller.update(
                target_power=target, current_power=current, dt=1.0
            )
        ...
        assert (
            output_variance < 30
        ), f"Output variance {output_variance} indicates instability"
    ...
    def test_anti_windup(self):
        ...
        assert (
            response_time <= 6
        ), "Anti-windup failed - integrator took too long to unwind"
