...
def test_parse_all_fields():
    ...
    frame = bytes(
        [
            0x1A,
            0x00,  # flags (bit 1,3,4 set = power, speed, cadence present)
            0x63,
            0x01,  # power: 355W (355 = 0x0163)
            0xB8,
            0x0B,  # speed: 3000 * 0.01 km/h = 30.00 km/h => 8.333 m/s
            0xAA,
            0x00,  # cadence: 170 (85rpm * 2) = 0x00AA
        ]
    )
...
def test_parse_missing_speed():
    ...
    frame = bytes(
        [
            0x12,
            0x00,  # flags (bit 1,4 set = power, cadence present)
            0xF4,
            0x01,  # power: 500W (500 = 0x01F4)
            0xB4,
            0x00,  # cadence: 180 (90rpm * 2) = 0x00B4
        ]
    )
